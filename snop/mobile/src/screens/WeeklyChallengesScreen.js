...
export default function WeeklyChallengesScreen({ route }) {
  ...
  useEffect(() => {
    if (route?.params?.challenge) {
      navigation.replace("WeeklyPractice", { challenge: route.params.challenge });
    }
  ...
  const handlePractice = (challenge) => {
    ...
    switch (type) {
      ...
      case "pronunciation":
      default:
        navigation.navigate("WeeklyPractice", { challenge });
        break;
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.backgroundSecondary }]}>
      <View style={[styles.container, { backgroundColor: colors.backgroundSecondary }]}>
        ...
        {/* Screen Title */}
        <Text style={[styles.screenTitle, { color: colors.primary }]}>Ukentlige utfordringer</Text>
