  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={{ flex: 1, padding: 16, backgroundColor: colors.background }}>
        ...
        <Text style={{ fontSize: 26, fontWeight: "800", color: colors.primary }}>{monthly?.title_no || monthly?.title}</Text>
        {monthly?.title_no && (
          <Text style={{ fontSize: 14, color: colors.textSecondary, fontStyle: "italic", marginTop: 2 }}>({monthly?.title})</Text>
      )}

      {/* Norwegian description (main) */}
      <Text style={{ fontSize: 16, color: colors.textPrimary, marginTop: 12, fontWeight: "500", lineHeight: 24 }}>{monthly?.description_no || monthly?.description}</Text>
      {monthly?.description_no && (
        <Text style={{ fontSize: 13, color: colors.textLight, fontStyle: "italic", marginTop: 4 }}>({monthly?.description})</Text>
      )}
      ...
      {monthly?.target && (
        <View style={{ marginTop: 20, backgroundColor: colors.backgroundAccent, padding: 18, borderRadius: 14, borderLeftWidth: 5, borderLeftColor: colors.accent, ...shadows.small }}>
          ...
          <Text style={{ fontSize: 24, color: colors.accent, fontWeight: "700", lineHeight: 32 }}>"{monthly.target}"</Text>
          <Text style={{ fontSize: 14, color: colors.textSecondary, fontStyle: "italic", marginTop: 10 }}>({monthly?.prompt})</Text>
        </View>
      )}
